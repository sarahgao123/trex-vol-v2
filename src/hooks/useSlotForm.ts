...
export function useSlotForm({ positionId, position, initialData }: UseSlotFormProps) {
  ...
  // Initialize volunteers from volunteer_list instead of volunteers
  const [volunteers, setVolunteers] = useState<Volunteer[]>(
    initialData?.volunteer_list?.map(v => ({
      email: v.user.email,
      name: v.name
    })) || []
  );
  ...
  const updateVolunteerName = (email: string, name: string) => {
    setVolunteers(volunteers.map(v => 
      v.email === email ? { ...v, name: name || null } : v
    ));
  };
